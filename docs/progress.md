...
| Layer 3 | 网络协议 | 29 | 25 | 29 | 29 | 10 | 179 | 204 |
| Layer 4 | 计算平台 | 29 | 25 | 29 | 29 | 10 | 190 | 215 |
| Layer 5 | 边缘智能 | 42 | 25 | 42 | 42 | 18 | 301 | 326 |
| Layer 6 | 安全与隐私 | 36 | 25 | 36 | 36 | 12 | 197 | 222 |
| Layer 7 | 综合应用 | 34 | 25 | 34 | 34 | 10 | 205 | 230 |
| Layer 8 | 前沿方向 | 33 | 25 | 33 | 33 | 10 | 248 | 273 |
| **合计** | | **697** | **200** | **697** | **697** | **90** | **1761** | **1964** |

- **内容文件 697**：可被 MkDocs 构建，不等于已完成来源审计。
- **显式导航 200**：当前 `mkdocs.yml` 直接列出的内容文件。
- **目录页入口 697**：八个层级 `catalog.md` 直接链接的内容文件。
- **可发现 697**：显式导航 ∪ 目录页链接。
- **层级首页入口 90**：八个概览页直接链接的内容文件。
- **Plan 条目 1761**：包含已执行和未执行计划，不能与现有文件直接相加。
- **来源审计**：current valid `STRUCTURAL` 结构审计 **642** 条，覆盖 **642** 个内容文件；事实核验：**645** 篇（`PARTIAL`/`VERIFIED`），`VERIFIED` **642** 篇。`STRUCTURAL` 只证明结构可审计，不代表技术事实已验证。
